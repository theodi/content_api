...
class ArtefactRequestTest < GovUkContentApiTest
  ...
  it "should return 404 if artefact has a live_at date in the future" do
    artefact = FactoryGirl.create(:my_non_publisher_artefact, state: 'draft', live_at: DateTime.now + 4.day)
    get "/#{artefact.slug}.json"
    assert last_response.not_found?
    assert_status_field "not found", last_response
  end
